...
/* ─────────────────────────────────────────
   Grille kente (motif décoratif arrière-plan)
───────────────────────────────────────── */
const KenteGrid = () => (
  <svg
    aria-hidden="true"
...
const Auth = () => {
  ...
  const handleAuth = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({
        title: "Erreur",
        description: error.response?.data?.email?.[0] || "Une erreur est survenue.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  /* ── Styles globaux injectés via <style> dans le head ── */
  const css = `
    ...
    .sk-tab-active { background: #C9A84C !important; color: #09090A !important; }
    .sk-tab:not(.sk-tab-active):hover { background: rgba(245,240,232,0.06); color: #F5F0E8; }
    ...
    @keyframes skSlideDown { from { opacity:0; max-height:0; } to { opacity:1; max-height:600px; } }
    .sk-slide { animation: skSlideDown 0.45s ease both; }
          ...
          >
            ...
            >
              {["login", "register"].map((mode, i) => (
                <button
                  key={mode}
                  className={`sk-tab ${isLogin === (mode === "login") ? "sk-tab-active" : ""}`}
                  ...
                  style={{
                    ...
                    transition: "all .2s", textTransform: "uppercase" as any,
                  }}
                >
            ...
            <form onSubmit={handleAuth} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {!isLogin && (
                <div className="sk-slide" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                      {roles.map(({ value, label, sub, icon: Icon }) => (
                        <button
                          ...
                          className={`sk-pill ${role === value ? "sk-pill-active" : ""}`}
                        ...
                        >
                          ...
                          <span style={{ fontSize: 9, letterSpacing: "0.1em", opacity: 0.6 }}>{sub}</span>
                        </button>
                      ))}
                    </div>
                  </div>
                  ...
                  {/* Boutique (SELLER) */}
                  {role === "SELLER" && (
                    <div className="sk-field sk-slide">
                      <label style={{
                        display: "block", fontSize: 10, letterSpacing: "0.22em",
                        textTransform: "uppercase" as any, color: "rgba(201,168,76,0.65)",
                        marginBottom: 7, fontWeight: 500,
                      }}>Nom de la boutique</label>
                      <GoldInput id="business_name" name="business_name"
                        placeholder="Ex: Ateliers d'Abidjan" required icon={Store} />
                    </div>
                  )}
                    ...
                    <div className="sk-slide" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      <div className="sk-field">
                        ...
                        <div style={{ position: "relative" }}>
                          ...
                          <select name="vehicle_type" defaultValue="MOTORCYCLE"
                            className="sk-select"
                            style={{ ...inp, padding: "13px 14px 13px 42px" }}>
                      ...
                      <div className="sk-field">
                        <label style={{
                          ...
                          textTransform: "uppercase" as any, color: "rgba(201,168,76,0.65)",
                          marginBottom: 7, fontWeight: 500,
                        }}>Zone de couverture</label>
                        <GoldInput id="coverage_area" name="coverage_area"
                          placeholder="Ex: Abidjan, Dakar, Lagos" required icon={Truck} />
                      </div>
                    </div>
                  )}

                  {/* Pièce d'identité */}
                  {(role === "SELLER" || role === "TRANSPORTER") && (
                    <div className="sk-slide">
                      <label style={{
                        display: "block", fontSize: 10, letterSpacing: "0.22em",
                        textTransform: "uppercase" as any, color: "rgba(201,168,76,0.65)",
                        marginBottom: 7, fontWeight: 500,
                      }}>Pièce d'identité (PDF / Image)</label>
